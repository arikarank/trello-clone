...
from models import db, User, Board, List, Card, BoardMember, FileAttachment
...
def get_card_attachments(card_id):
    try:
        ...
        attachments = FileAttachment.query.filter_by(card_id=card_id).order_by(FileAttachment.uploaded_at.desc()).all()
...
def create_checklist_item(checklist_id):
    try:
        ...
        last_item = ChecklistItem.query.filter_by(checklist_id=checklist_id).order_by(ChecklistItem.position.desc()).first()
...
# Update the get_card route to include checklists
@app.route('/api/cards/<int:card_id>')
@login_required
def get_card(card_id):
    try:
        ...
        for checklist in checklists:
            items = ChecklistItem.query.filter_by(checklist_id=checklist.id).order_by(ChecklistItem.position).all()
            items_data = []
            completed_count = 0
...
def add_label_to_card(card_id, label_id):
    try:
        ...
        if not has_board_access(card.list.board_id, current_user.id) or not has_board_access(label.board_id, current_user.id):
            return jsonify({'error': 'Access denied'}), 403
...
# Update the get_card route to include labels
@app.route('/api/cards/<int:card_id>')
@login_required
def get_card(card_id):
    try:
...
# Update the get_board route to include labels in cards
@app.route('/api/boards/<int:board_id>')
@login_required
def get_board(board_id):
    try:
...
def search():
    try:
        ...
        # Search in cards (title, description, checklist items)
        ...
        for label in label_results:
            ...
            matching_cards = Card.query.join(card_labels).filter(
                card_labels.c.label_id == label.id,
                List.board_id.in_(board_ids)
            ).all()
            
            search_results['cards'].extend([{
                'id': card.id,
                'title': card.title,
                'description': card.description,
                'type': 'card',
...
def get_highlighted_fields(item, query):
    ...
    if hasattr(item, 'title') and query_lower in item.title.lower():
